...
const download = (url) => <a href={url} target='_blank' className='download'>
...
export default ({ url, size }) =>
  ext.video.includes(url.split('.').pop())
    ? <Video url={url} size={size}/>
    : <div className={[ 'attachment', ext.audio.includes(url.split('.').pop()) && 'audio' ].filter(Boolean).join(' ')}>
      <div className='data'>
        <img src={`https://discordapp.com/assets/${fileToIcon(url)}.svg`} alt='' className='icon'/>
        <div className='details'>
